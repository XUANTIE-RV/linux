...
static struct platform_device ultrafb_device = {
	.name = "viv-dc",
	.id = -1,
};
...
static int __init ultrafb_driver_init(void)
{
	int ret = 0;
	ret = platform_device_register(&ultrafb_device);
	if (ret < 0)
		return ret;

	ret = platform_driver_register(&ultrafb_driver);
	if (ret < 0)
		return ret;
...
static void __exit ultrafb_driver_exit(void)
{
	platform_device_unregister(&ultrafb_device);
	platform_driver_unregister(&ultrafb_driver);
}

module_init(ultrafb_driver_init);
module_exit(ultrafb_driver_exit);
