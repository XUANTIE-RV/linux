...
#define DH_PRINT printf
    ...
    #define DH_TRACE DH_MDK_SYS_LogWrite
#else
    #define DH_ASSERT(expr)
...
#define DH_DBG(fmt, args ...) \
	do \
	{ \
		DH_PRINT(PURPLE "DEBUG (%s|%d): " fmt NONE_C, __func__, __LINE__, ## args); \
	} \
	while(0)
...
#define DH_ERROR(fmt, args ...) \
    do \
    { \
    	DH_PRINT(RED "ERROR (%s|%d): " fmt NONE_C, __func__, __LINE__, ## args); \
    } \
    while(0)
...
#define DH_WARN(fmt, args ...) \
	do \
	{ \
		DH_PRINT(YELLOW "WARN (%s|%d): " fmt NONE_C, __func__, __LINE__, ## args); \
	} \
	while(0)
...
#define DH_INFO(fmt, args ...) \
	do \
	{ \
		DH_PRINT(GREEN "INFO (%s|%d): " fmt NONE_C, __func__, __LINE__, ## args); \
	} \
	while(0)
